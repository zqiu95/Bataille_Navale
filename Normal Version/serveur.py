...
def fun_msgToClient_askPseudo(mySocket,data,sockIndex):
    ...
    # New Client
        ...
        print (pr)
        mySocket.send(("<Auto> User commands : CMD JOIN MSG QUIT MORE \"for more information\"").encode())
        if (startGame==1):
            mySocket.send(("\n<Auto> Game already started : You're watcher. ").encode())
            mySocket.send(("\n Waiting for players moves... [88]").encode())
...
def fun_msgToOtherThanCurrentPlayer():
    ...
    for selectedClient in l_client:
        if selectedClient.get_status() == waitingPlayer:
            print("loop")
            selectedClient.get_socketName().send(("<Auto> Waiting opponents move [88]...").encode())
    ...
    for selectedClient in l_client:
        ...
        if (l_client.index(selectedClient)!=indexSocketDeconnected):
            if selectedClient.get_status() == 1 or selectedClient.get_status()== 0 :
                selectedClient.get_socketName().send(("<Auto> A watcher join the game. Waiting for player return [88]...").encode())
            elif selectedClient.get_status() == watcher:
...
def fun_gameImplementation(mySocket,index):
    ...
    if startGame == 0 and (not index in playerReady) :
        ...
        if nbPlayer == 1:
            ...
            mySocket.send(("<Auto> You joined a game. Waiting for opponents [88]").encode())
        ...
        if nbPlayer == 2:
            ...
            mySocket.send(("<Auto> You joined a game. The game will start soon [88]").encode())
            copyGame = init ()
            myGame = copyGame
            ...
            l_client[playerReady[0]].get_socketName().send(("<Auto>\n"+grid+"\n<Auto> Which column?").encode())
...
def fun_gameplay(mySocket,data):
    ...
    if columnLine == column:
        ...
        # Wrong x given --> loop column continue
            ...
            grid=displayConfiguration2(myGame.boats[player],myGame.shots[otherPlayer],myGame.shots[player])
            mySocket.send(("<Auto>\n"+grid+"\n<Auto> Which column?").encode())
    elif columnLine == line:
        if fun_checkLine(data)==True:
            y=fun_valueLine(data)
            ...
            if (gameOver(myGame)!=-1):
                l_client[playerReady[otherPlayer]].get_socketName().send(("<Auto> YOU WIN [88]").encode())
                l_client[playerReady[player]].get_socketName().send(("<Auto> YOU LOSE [88]\n").encode())
                ...
                for tmpSocket in l_socket:
                    tmpSocket.send(("<Auto> User commands : CMD JOIN MSG QUIT MORE \"for more information\"").encode())
                return False
            ...
            grid=displayConfiguration2(myGame.boats[player],myGame.shots[otherPlayer],myGame.shots[player])
            l_client[playerReady[player]].get_socketName().send(("<Auto>\n"+grid+"\n<Auto> Which column?").encode())
            fun_msgToOtherThanCurrentPlayer()
            ...
            if len(data) == 0:
                ...
                if (b_newClient or fun_indexSocket(l_client,selectedSocket)==-1) and fun_checkReconnectionClient(selectedSocket,data):
                    ...
                    for c2 in l_client:
                        if c2.get_pseudoClient()==pseudoClientDeco:
                            ...
                            if (c2.get_status()==0):
                                selectedSocket.send(("<Auto>\n"+grid+"\n<Auto> Which column?").encode())
                                for c4 in l_client:
                                    s=c4.get_socketName()
                                    if c4.get_status()==1:
                                        s.send(("<Auto> Other player reconnected\n waiting opponents move [88] ").encode())
                ...
                #==== Not a reconnection
                    ...
                    #Player sending stuff
                        ...
                        tmpStatusC=tmpC.get_status()
                        if "STOP" in str(data) and (tmpStatusC==0 or tmpStatusC==1):
                            fun_clientSTOP(selectedSocket,socketServeur)


            elif l_client[index].get_pseudoClient()=="-1":
                print("====pseudoLOOP")
                print(l_client[index].get_pseudoClient())
                ...
                selectedSocket.send(("<Auto> User commands : CMD JOIN MSG QUIT MORE \"for more information\"").encode())
                ...
                selectedSocket.send(("yolo1234").encode())
